...
def main(args):
  ...
  if args.question is not None and args.image_h5 is not None:
    run_raw_images(args, model)
  else:
...
def run_raw_images(args, model):
  ...
  for imgs in tqdm(dataloader):
    ...
    if type(model) is tuple:
      ...
      scores = execution_engine(feats_var, torch.vstack([predicted_program] * imgs.size(0)))
    else:
      model.type(dtype)
      scores = model(torch.vstack([question_var] * imgs.size(0)), feats_var)
