...
class CIFAR10DataSplit(pl.LightningDataModule):
    def __init__(self, args):
        super().__init__()
        self.hparams = args
        self.mean = (0.4914, 0.4822, 0.4465)
    ...
    def splits_dataset(self, dataset, split):
        batch_size = 256
        cifar_dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False,
                                      num_workers=self.hparams.num_workers)
        label_indexes =  [[] for _ in range(10)]
        ...
        random = np.random.RandomState(self.hparams.split_seed)
        subset_index = []
        for indexes in label_indexes:
            random.shuffle(indexes)
            subset_index.extend(np.split(indexes, 2)[split])
        return Subset(dataset, subset_index)
    ...
    def train_dataloader(self):
        ...
        dataset = CIFAR10(root=self.hparams.data_dir, train=True, transform=transform)
        subset = self.splits_dataset(dataset, self.hparams.split)
        dataloader = DataLoader(
            subset,
            batch_size=self.hparams.batch_size,
            num_workers=self.hparams.num_workers,
